...
export const ROLE_OPTIONS = [
  // Un serveur appartient à UN seul pôle, choisi juste en dessous dans le
  // formulaire : restaurant ou bar, jamais les deux.
  ['serveur', 'Serveur'],
  ['reception', 'Réception'],
  ['resto', 'Restauration'],
  ['bar', 'Bar'],
...
// ---------- Serveurs : secteurs, assignation, accusé de réception ----------

export const SECTOR_LABELS = { resto: 'Restaurant', bar: 'Bar' };
...
// Le serveur d'un pôle gère les tables du restaurant ou les salons du bar.
// Les chambres passent par la réception : elles n'ont pas de serveur attitré.
export const SECTOR_QR_TYPE = { resto: 'table', bar: 'salon' };

// Les règles de répartition vivent dans un fichier sans dépendances, pour
// pouvoir être testées seules (voir scripts/test-serveur.mjs).
export { ACK_DEADLINE_MS, splitServeurOrders } from './serveurOrders.mjs';

// Chrono « 0:07 » / « 2:14 » à partir d'un nombre de millisecondes
...
// Détail du règlement sur le ticket. Un paiement mixte affiche ses deux
// parts, pour que le client voie ce qu'il a donné en liquide et en MoMo.
function paiementTicket(order) {
  if (!order.paid_at) {
    return '<div class="sep"></div><p class="center note">Non réglée</p>';
  }
  const fcfa = (value) => `${Number(value || 0).toLocaleString('fr-FR')} FCFA`;
  const lignes = [];
  if (order.payment_method === 'mixed') {
    lignes.push(`<tr><td>Part Espèces</td><td style="text-align:right">${fcfa(order.cash_amount)}</td></tr>`);
    lignes.push(`<tr><td>Part MoMo</td><td style="text-align:right">${fcfa(order.momo_amount)}</td></tr>`);
  } else if (order.payment_method === 'mobile_money') {
    lignes.push(`<tr><td>Mobile Money</td><td style="text-align:right">${fcfa(order.momo_amount)}</td></tr>`);
  } else {
    lignes.push(`<tr><td>Espèces</td><td style="text-align:right">${fcfa(order.cash_amount)}</td></tr>`);
  }
  return `<div class="sep"></div><table>${lignes.join('')}</table>`;
}
...
// La librairie de génération de QR codes ne sert qu'aux écrans « QR codes ».
// On la charge à la demande pour ne pas l'imposer à tous les postes.
