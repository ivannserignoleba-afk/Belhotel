...
// Deux chemins indépendants pour maximiser la fiabilité (surtout mobile) :
// un vrai élément <audio> (WAV synthétisé) + Web Audio en complément.
// Les deux sont déverrouillés au premier geste puis réutilisables.
let audioContext = null;
let beepAudio = null;
let soundUnlocked = false;

// Génère un « bip-bip » (WAV 16 bits) sous forme d'URL Blob, sans fichier externe
function makeBeepUrl() {
  ...
  for (let i = 0; i < total; i += 1) {
    ...
    const seg = t % 0.25; // deux segments de 0,25 s
    const freq = seg < 0.125 ? 988 : 740;
    const local = seg % 0.125;
    const env = Math.max(0, Math.min(local * 34, (0.125 - local) * 34, 1));
    const sample = Math.sin(2 * Math.PI * freq * t) * env * 0.9;
    view.setInt16(pos, Math.max(-1, Math.min(1, sample)) * 32767, true);
    pos += 2;
  }
...
function webAudioBeep(times) {
  try {
    ...
    for (let index = 0; index < times; index += 1) {
      ...
      [[988, 0], [740, 0.14]].forEach(([frequency, offset]) => {
        ...
        gain.gain.setValueAtTime(0.55, base + offset);
...
export function beep(times = 3) {
  ensureAudio();
  // Chemin 1 : élément <audio> (le plus fiable une fois déverrouillé)
  if (beepAudio) {
    let played = 0;
    const playOnce = () => {
      try {
    ...
    if (times > 1) {
      const timer = setInterval(() => {
        played += 1;
        playOnce();
        if (played >= times - 1) clearInterval(timer);
      }, 620);
    }
  }
  // Chemin 2 : Web Audio en complément (démarrage instantané, Android)
  webAudioBeep(times);
}
